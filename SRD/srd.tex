...
\section{Interface graphique (G.U.I.)}
	...
	Nous verrons brièvement l'interface du menu console, étant peu innovant et se résumant toujours à la même chose, un seul exemple est plus que suffisant.
	\subsection{Mise en place de l'interface des différents menus.}
		...
		C'est pourquoi la possibilité d'utiliser un interface grpahique a été implémentée.
		...
		Maintenons, obersons chacun de ces onglets.

		\newpage
		\subsubsection{Commencer une partie.}
		\begin{center}\includegraphics[scale=0.5]{Lobby.png}\end{center}
		Il suffit simplement de sélectionner \textit{Find a Game} pour que le serveur réunissent deux jours en attente pour commencer une partie.
		...
		Tout est dans le titre, ici nous pouvons observer les cartes que nous possedons.
		...
		Quand à la deuxième moitié de la carte, la partie inférieure est composée des points de vie du monstres (l'icone coeur), son attaque (les deux épées croisées), du nombres de points de boucliés que la créature possède aisin que son type de bouclier définit par la couleur du nombre (bleu, noir, orange) et également de ses effets si elle en a.
		...
		La classification est simple, c'est le joueur possédant le ratio le plus proche ou égale à qui est premier.
