...
\section{Introduction}
	\subsection{But du projet}
		...
		\subsubsection{Description du Wizard Poker}
			...
			Le premier joueur sera choisi aléatoirement par le serveur. Ensuite, tout à tour, les joueurs pourront utiliser leurs cartes, qu'elles soient un \gls{sort} ou une \gls{creature} tant que leur coût d'utilisation/d'invocation/, ne dépasse pas l'énergie disponible du joueur. ( Le joueur gagne un point d'énergie supplémentaire par tour jusqu'à atteindre son maximum de 10.)
			
			Les cartes peuvent attaquer soit l'adversaire directmeent, soit une , soit une \gls{creature} adverse. Ou encore avoir un ou plusieurs effets \textit{spéciaux}\footnote{Par exemple : \gls{invoquer} une carte au hasard, redonner de la vie, augmenter l'attaque, etc.}. 
		...
		\subsubsection{Possibilités d'actions de l'utilisateur}
			...
			\begin{itemize}
				...
				\begin{description}
					...
					\item[Pré-condition] L'utilisateur a un \gls{deck} actif.
...
\section{Besoins du système}
	...
	\subsection{Design et fonctionnement du système}
		...
		\begin{center}\includegraphics[scale=0.5]{deck.png}\end{center}
			...
			soit des \glspl{sort} et on ne peut posséder que maximum 2 exemplaire d'une même carte au sein d'un même \gls{deck}.
			A savoir également que les\glspl{deck} sont indépendants, il est donc possible de posséder uniquement 2x la même carte, mais avoir plusieurs\glspl{deck} qui possèdent 1 ou 2x cette carte.
		...
		\subsubsection{Diagramme des classes nécessaires au menu en console}
			...
			\begin{center}\includegraphics[scale=0.5]{stack1.png}\end{center}
				...
				En effet, pour aller plus loin dans le menu, il suffit de "\gls{push}" notre choix, et pour un retour en arrière il suffit de faire un "\gls{pop}", et le tour est joué. 
